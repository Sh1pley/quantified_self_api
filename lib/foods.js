...
function modifyFood() {
  $('span').on('click',function(event){
    var className = event.currentTarget.offsetParent.className
    var foodName = this.innerText;
    changeFood(className, foodName);
  });
}

function changeFood(className, foodName) {
  $('span').keydown(function(){
    if (event.keyCode == 13) {
      console.log(className)
      console.log(foodName)
      var $foodRow = $(this).parent().parent().children();
