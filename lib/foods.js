...
function changeFood() {
  var foodName;
  $('span').on('focus',function(event){
    foodName = $(this).parents('tr.food-row').children('td.food-name').text();
  });
  $('span').keydown(function(event){
    if (event.keyCode == 13) {
      $('span').blur();
      updateFood(foodName, $(this).text());
      return false;
    }
  });
}

function updateFood(foodName, newValue){
  if ($(this).parents('td.food-calories').text()) {
